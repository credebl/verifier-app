...
function UploadRecords({stepChange}: IProps) {
    const initOption = [
        {
            label: "Select Schema - Credential definition",
            value: ""
        }
    ]
    const [credentialSelected, setCredentialSelected] = useState('');
    const [credentialOptions, setCredentialOptions] = useState(initOption);
    ...
    const getCredentialOptions = async () => {
        ...
        try {
            ...
            const options = res && res.length > 0 && res.map((item: { schemaName: any; schemaVersion: any; credentialDefinition: any; credentialDefinitionId: any; }) => {
                const label = `${item.schemaName} [${item.schemaVersion}] - (${item.credentialDefinition})`
                return {
                    ...item,
                    label,
    ...
    const DownloadSchemaTemplate = async () => {
        ...
        if (credentialSelected) {
            try {
                ...
                if (response) {
                    const url = encodeURIComponent(response)
    ...
    const handleFileUpload = async (file: any) => {
        ...
        try {
            ...
            // const blob = new Blob([new Uint8Array(binaryData.buffer)]);

            // body.append("file", blob, fileLabel);

            // file
            // console.log(23423421, body);
            ...
            // postdata.append('data', new Blob([binaryData], { type: "text/plain" }), `${fileLabel}`);
            console.log(2342111, data);
            await wait(500);

            // const data = new URLSearchParams();
            // for (const pair of new FormData(payload)) {
            //     data.append(pair[0], pair[1]);
            // }

            console.log(797897, payload);
            const response = await uploadCsvFile(token, payload, credentialSelected);
            console.log(2342341, response, credentialSelected)
            if (response?.data) {
    ...
    return (
        <div className="p-4 sm:px-8 sm:py-12">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 p-6">
                        <div className="">
                            <div className="search-dropdown text-blue-800 dark:text-blue-800">
                                <Select placeholder="Select Schema - Credential definition"
                                                    ...
                                                    csvData.map((row, rowIndex) => (
                                                        ...
                                                        >
                                                            ...
                                                            {/* <td className={`p-4 text-sm font-normal text-gray-900 whitespace-nowrap dark:text-white align-middle`}>
                                                                <button
                                                                    className={`py-2 px-4 rounded-md inline-flex items-center border text-sm min-w-[7.1rem] h-[2.4rem] text-blue-800 dark:text-white border-blue-800 bg-white-700 hover:text-white hover:bg-blue-800`}>
                                                                    Edit Records
                                                                </button>
                                                            </td> */}
                                                        </tr>
